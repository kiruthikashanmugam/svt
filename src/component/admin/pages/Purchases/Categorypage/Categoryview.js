...
function Categoryview(props) {
  ...
  async function searchdata(query = '') {
    try {
      const response = await axios.get(`https://svt.know3.com/api/category_list?q=${query}`);
  ...
  const onDelete = (_id) => {
    axios
      .get(`https://svt.know3.com/api/delete_categorylist/${_id}`)
      .then(() => {
        searchdata();
      });
  };
    ...
    const filteredRecords = originaldata.filter((record) => {
      return (
        ...
        (record.Category_Name &&
          record.Category_Name.toString().includes(query)) ||
        (record.Sub_category &&
  ...
  const renderTableRows = () => {
    ...
    const filteredRecords = currentRecords.filter((record) => {
      return (
        ...
        (record.Category_Name &&
          record.Category_Name.toString().includes(searchQuery)) ||
        (record.Sub_category &&
    ...
    return filteredRecords.map((use) => (
      <tr key={use._id} style={{ textAlign: "center" }}>
        ...
        <td>
          <Link style={{ color: '#000000' }} to={`/admin/editcategory/${use._id}`} onClick={() => setData(data)}>
    ...
    <div>
      ...
      <Container>
        ...
        <div className="flexbox-container" >
          ...
          <div>
            <h1 style={{ paddingRight: '10px', paddingTop: '5px', fontSize: '20px', fontWeight: 'bold', color: '#000000' }}>Category List</h1>
          </div>
          <div >
